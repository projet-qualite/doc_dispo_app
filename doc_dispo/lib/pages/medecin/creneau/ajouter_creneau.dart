                        ...
                        InkWell(
                            onTap: () {
                              if (_formKey.currentState!.validate()) {
                                if (!val!.isEmpty) {
                                  List<Creneau> list =
                                      getCreneauOfMedecin(currentUser.id);

                                  for (int i = 0; i < val!.length; i++) {
                                    String? heure = val![i].toString();
                                    int motif = listMotif
                                        .where((element) =>
                                            element["motif"].libelle ==
                                            default_value)
                                        .toList()[0]["id_motif_consult"];
                                    int id = 6 + i;

                                    creerCreneau(Creneau(
                                        id: 6 + i,
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                        content: Text(
                                            'Vous devez sélectionner au moins une heure')),
                                  );
                                }
                              }
                            },
