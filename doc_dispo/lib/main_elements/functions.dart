...
List<Map<String, dynamic>> getRdvProche(int id) {
  ...
  list_proche.forEach((key, proche) {
    if (proche.id_patient == id) {
      list_rdv_futur.forEach((key2, rdv) {
        if (rdv.id_proche == proche.id) {
          Medecin? medecin;
          Specialite? specialite;
          Creneau? creneau = list_all_creneau[rdv.id_creneau];
          if (creneau != null) {
            medecin = list_medecin[creneau.id_medecin];
            specialite = list_specialite[medecin!.id_specialite!];
            list.add({
              "medecin": medecin,
              "creneau": creneau,
              "specialite": specialite,
...
List<Map<String, dynamic>> getRdvPasseProche(int id) {
  ...
  list_proche.forEach((key, proche) {
    if (proche.id_patient == id) {
      list_rdv_passe.forEach((key2, rdv) {
        if (rdv.id_proche == proche.id) {
          Medecin? medecin;
          Specialite? specialite;
          Creneau? creneau = list_all_creneau[rdv.id_creneau];
...
List<Map<String, dynamic>> getRdvMedecins(int id) {
  ...
  list_rdv_futur.forEach((key, rdv) {
    Creneau? creneau = list_all_creneau[rdv.id_creneau];
    if (creneau != null) {
      if (creneau!.id_medecin == currentUser.id) {
        Proche? proche = list_proche[rdv.id_proche];
        list.add(
            {"proche": proche, "creneau": creneau, "date_naissance": proche!.date_naissance});
      }
    }
  });
...
List<Map<String, dynamic>> getRdvPasseMedecins(int id) {
  List<Map<String, dynamic>> list = [];
  list_rdv_passe.forEach((key, rdv) {
    Creneau? creneau = list_all_creneau[rdv.id_creneau];
    if (creneau != null) {
      if (creneau!.id_medecin == currentUser.id) {
        Proche? proche = list_proche[rdv.id_proche];
...
bool definedOwner(int id)
{
  List<Proche> list = getProcheOfPatient(id);
...
List<Creneau> getCreneauOfMedecin(int id) {
  ...
  list_creneau.forEach((key, creneau) {
    if (creneau.id_medecin == id && creneau.etat == 0) {
...
List<Hopital> getAllInformationsAssurance({required int id}) {
  ...
  list_affilier.forEach((element) {
    if (element.id_assurance == id) {
      listHopitaux.add(list_hopital[element.id_hopital] ??
          Hopital(
              id: -1,
              slug: "",
              email: "",
...
Map<String, List<Creneau>> getAllCreneauOfMedecin(
    {required Medecin medecin, required int id_motif_consultation}) {
  Map<String, List<Creneau>> creneaux = {};

  list_creneau.forEach((key, creneau) {
    if (creneau.id_medecin == medecin.id &&
        creneau.etat == 0 &&
        (creneau.id_motif_consult == 0 ||
            creneau.id_motif_consult == id_motif_consultation)) {
...
Map<String, List<Creneau>> trierHeureParOrdreCroissant(
    {required Map<String, List<Creneau>> creneaux}) {
  creneaux.forEach((key, value) {
    creneaux[key]!.sort((Creneau a, Creneau b) =>
        double.parse(a.heure).compareTo(double.parse(b.heure)));
  });
...
String showDate(String dateFormate) {
  ...
  return jour.substring(0, 1).toUpperCase() +
      jour.substring(1) +
      " " +
      mois +
      " " +
...
String dateRdv(String date, String hour) {
  String dateFormate =
      DateFormat.yMMMMEEEEd("fr_FR").format(DateTime.parse(date));

  String jourS = dateFormate.split(" ")[0];
  String numeroJour = dateFormate.split(" ")[1];
  String mois = dateFormate.split(" ")[2];

  return jourS.substring(0, 1).toUpperCase() +
      jourS.substring(1, 3) +
      ", " +
      numeroJour +
      " " +
      mois.substring(0, 1).toUpperCase() +
      mois.substring(1, 3) +
      ", " +
      showHour(hour);
}
